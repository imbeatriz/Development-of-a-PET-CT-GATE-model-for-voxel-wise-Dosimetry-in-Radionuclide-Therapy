...
def run_calculation():
    ...
    if not dose_path or not unc_path or not voi_folder:
        messagebox.showerror("Error", "Please select Dose file path, Statistical Uncertainty file path and VOI folder.")
        return
    ...
    try:
        ...
        if dose_array.shape != unc_array.shape:
            messagebox.showerror("Error", "Dose and Statistical Uncertainty image shapes do not match!")
            return
