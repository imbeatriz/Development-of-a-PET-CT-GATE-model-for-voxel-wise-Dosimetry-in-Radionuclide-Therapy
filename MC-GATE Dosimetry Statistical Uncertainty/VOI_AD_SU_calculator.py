...
def run_calculation():
    ...
    try:
        ...
        mean_dose = np.mean(dose_array[voi_mask])  # Gy

        # --- Mean relative uncertainty in VOI ---
        mean_rel_unc = np.mean(unc_array[voi_mask])  # dimensionless

        # --- Absolute uncertainty of mean dose ---
        abs_unc = mean_dose * mean_rel_unc  # Gy

        # --- Relative uncertainty (%) ---
        rel_unc_percent = mean_rel_unc * 100


        # --- Show results in a popup window ---
        messagebox.showinfo(
        "Results",
        f"Mean absorbed dose in VOI: {mean_dose:.2f} Gy\n"
        f"Absolute statistical uncertainty (±): {abs_unc:.2f} Gy\n"
        f"Relative statistical uncertainty: {rel_unc_percent:.2f} %"
)
